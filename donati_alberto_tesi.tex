...
\begin{abstract}
    Considering that companies have an increasing number of servers connected to the network, IT professionals find new ways to execute the same task multiple times.
    Therefore, systems that can automate tasks on large numbers of machines in parallel are created and are increasingly being used.
    ...
    Technologies considered are based on open source projects, with a free version and a community-oriented license.
    Closed technology properties for example tools of Amazon, Google and Microsoft are not considered.
    The thesis analyzes for each technology the historical background, the purpose of that, the community, the main components and some characteristics.
    Characteristics are the platforms available, the project itself, the architecture and the extensions. Also, it describes some information about the installation and the test of the technology.
    There are, in the end, some tables and explanations for a comparison point of view of the technologies analyzed.
    It is provided also a summary overview of the reasons why it could choose one alternative over another.
\end{abstract}
...
%----------------------------------------------------------------------------------------
Nowadays IT companies need technologies to manage the infrastructure they have, which simplifies and organizes the jobs on servers.
The number of servers is increasing year after year, computing capacity is continually growing, and technologies are advancing.
...
Starting from a previous internship experience in the DevOps field, I became interested in automation tools.
During my experience, I saw and learned to use Ansible among the available tools.
With Ansible, I learned how to create several commands to perform automations that also included launching executions of personalized scripts.
Using this experience with Ansible as a starting point, I decided to go deeper into the topic on Ansible and other open source and free automation tools.
This document aims to study and compare some open source automation tools, that have a wide community and a free version.
Automation tools are not useful only for wide organizations but could be used also for personal projects or small businesses.
This thesis is useful for IT students, teachers and enthusiasts who wish to manage their IT environment differently.
It is a starting point to see what is an automation tool and compare differences and similarities in structures, communities, technologies, and ways to automate with these tools.
Providing an overview and comparison of them helps users to have a summarized point of view of these technologies and what could be the correct choice for their use case.

\section{Methodology}
There is a focus on tools created in these years. Searching for technologies just created is not the scope, and also finding technologies too old might result in not being useful for real use.

Technologies based on open source projects with a free version and a community-oriented license are considered.

There are some technologies properties of Amazon, Google and Microsoft, even if they might be described, they are not the focus of the document.

That study also searches for technologies that are useful for multipurpose scope, always related to automation.

These tools have a solid user base. Also, It does not search for a particular language or a single way to manage machines (for example agent-agentless).

There are no limitations about the languages used in the tools for automation or about the language for the tool project itself. Some base automations on YAML language (Salt, Ansible), and others on Ruby and/or relative DSL (Chef, Puppet).

\section{Explanation of the structure}
Initially, there is a brief description of what the automation tools are and who uses them.

Also, there is a brief explanation about Open Source, and its importance for the projects in our use case.

For each technology, there is a little historical view, a little study about the open source project, the trend of the language, the community and the support.

There is a description of the tool and its components.

Also, some info on installation and examples.

Considering that automation can be easily integrated with CI/CD, it searches for some sort of automatic test and checks of the technology.

Considering this research and other sources, there is a little description of the pros and cons.

At the end, there is a summary of the research done writing that document, and tables or similar mixing different technologies.

The structure includes ad the beginning the introduction and the state of the art.
After, for each technology is indicated at least the history, the community, main components, installation and ways of testing.
At the end, there is a conclusion chapter with comparisons and considerations.
...
Some of the tools used by the community for automation, which try to reply to the scope indicated above are \textbf{Ansible, Salt, Chef, Puppet}.
...
\textbf{What}(What purpose they were created for), \textbf{Where}(Where the machines are located) and \textbf{When}(The stage of development they are assigned to).
...
Considering that Ansible does not have active agents, this does not consume extra resources in the hosts where the automatisms are executed. Ansible takes space and resources only during the execution of the activity on the controlled node.

Ansible has a fast configuration, not having agents that must be constantly active on the host machines. Ansible connects directly to the host machines via SSH (in Linux), and it is enough to have Python installed on the host machines.
...
The machine that controls others connects to nodes via SSH connections in Linux. The nodes also need (to run almost all Ansible commands) Python installed.
...
\subsection{History}
Salt was started in 2011, as a project ideated by Thomas S. Hatch which was not satisfied with other open source technologies available~\cite{saltFloss}. He decided to use the fast ZeroMQ for connections and messages between machines.


For example, he posted about \textbf{Salt Configuration Management} when he wrote:


"The Salt configuration management took a big leap forward late last night. The capability to define more complex states in salt from the master became a reality. Salt can now define states on the master file server, and pull those states on the minions. More work still needs to be done, and we still need to write the documentation, but this is a big step forward!"~\cite{saltConf}.
...
In a node that acts as a master node, it seems reading the documentation that only in (many) Linux systems can do that.
...
Salt works on Unix and also on Windows systems, but also on different types of network devices such as switches and routers from different brands.
It is usually used in a mode with agents called minions, but has also a limited working mode agentless.
It works with agents who use ZeroMQ and msgpack to communicate. The agent approach with good technology made the communication where there is better knowledge about the state of the nodes.
Salt uses public keys for authentication and AES encryption for payload communication.
...
That is available at \url{https://github.com/chef/chef-oss-practices}, which includes also a guide.
...
Chef works on different Linux distros, MacOS and Windows and it is thought to work on clouds, on-premise and also on hybrid systems.
...
Puppet gives some benefits (like discount, contents, stickers) to the contributors that are in a "group" called Puppet Champion". 
There is also a special group called "The League of Extraordinary Puppeteers" that can you enter on it only by community nomination (and offers additional benefits). Puppet is DevOps oriented because Puppet wishes to work in an agile style inviting system administration to use devops technologies and practices.
Puppet has a strong user base, used by from community users to big tech companies (Accenture, Uber, Twitch), it has been created years before different competitors.
